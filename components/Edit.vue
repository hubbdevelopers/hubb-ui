<template>
  <section class="section">
    <div class="container">
		<div v-if="canEdit">
			編集可能です
			<button class="button is-primary" @click="saveContent">保存</button>
			<input type="checkbox" v-model="page.Draft" id="draft" ><label for="draft">下書きとして保存</label>
			<div>
				<div class="field">
					<croppa :v-model="myCroppa" @file-choose="handleCroppaFileChoose" />
					<label class="label">ページ名</label>
					<div class="control">
						<input class="input" type="text" placeholder="Text input" v-model="page.Name">
					</div>
				</div>
			...
			<nuxt-link :to="'/' + $store.state.user.accountId + '/' + page.ID">戻る</nuxt-link>
		</div>
	</div>
  </section>
</template>
