  ...
  <section class="section">
    <div class="container">
      ...
      <div class="columns is-multiline">
        ...
        >
          <list-user-box :user="member" />
        </div>
      </div>

      <h1 v-if="isOwner" class="title">招待中一覧</h1>
      <div class="columns is-multiline">
        <div
          ...
          class="column is-one-quarter"
        >
          <user-invitation-box :user="invitation" :community="community" />
        </div>
      </div>
      <div v-if="isOwner">
        <button
...
export default {
  ...
  computed: {
    ...
    isFollowingCommunity: function() {
      return this.$store.getters['user/isFollowingCommunity'](
  ...
  async created() {
    ...
    this.pages = (await this.$axios.$get(
      `pages?communityid=${this.community.ID}`
    )).data
  },
  methods: {
    ...
    addMember: function() {
      ...
      const invitationRef = communitiesRef
        .doc(this.$route.params.communityId)
    ...
    follow() {
      this.$store
        .dispatch('user/followCommunity', this.$route.params.communityId)
        .catch(err => {
          console.log(err)
          window.alert('error')
        })
    ...
    unFollow() {
      this.$store
        .dispatch('user/unFollowCommunity', this.$route.params.communityId)
        .catch(err => {
          console.log(err)
          window.alert('error')
        })
    ...
    deleteCommunity() {
      ...
      this.$store
        .dispatch('user/deleteCommunity', this.$route.params.communityId)
        .then(() => {
